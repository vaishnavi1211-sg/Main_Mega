...
class DashboardProvider with ChangeNotifier {
  ...
  Future<void> _fetchDashboardData() async {
    try {
      ...
      // If no top products from API, generate sample data
      if (_dashboardData.topProducts.isEmpty) {
        _dashboardData = DashboardData(
          totalRevenue: _dashboardData.totalRevenue,
          totalOrders: _dashboardData.totalOrders,
          pendingOrders: _dashboardData.pendingOrders,
          activeEmployees: _dashboardData.activeEmployees,
          revenueGrowth: _dashboardData.revenueGrowth,
          orderGrowth: _dashboardData.orderGrowth,
          employeeGrowth: _dashboardData.employeeGrowth,
          topProducts: _getSampleTopProducts(),
          recentActivities: _dashboardData.recentActivities,
        );
      }
    } catch (e) {
      ...
      _dashboardData = DashboardData(
        ...
        employeeGrowth: 5.2,
        topProducts: _getSampleTopProducts(),
        recentActivities: _getSampleRecentActivities(),
      );
    }
  }
  ...
  Future<void> _fetchChartData() async {
    try {
      ...
      // If no chart data from API, generate sample data
      if (_revenueChartData.isEmpty) {
        _revenueChartData = _getSampleChartData();
      }
    } catch (e) {
      debugPrint('Error fetching chart: $e');
      _revenueChartData = _getSampleChartData(); // Set sample data as fallback
    }
  }

  List<Map<String, dynamic>> _getSampleChartData() {
    final now = DateTime.now();
    final days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    
    List<Map<String, dynamic>> chartData = [];
    int baseRevenue = 150000;
    
    for (int i = 6; i >= 0; i--) {
      final date = now.subtract(Duration(days: i));
      final dayName = days[date.weekday - 1];
      // Generate revenue with some variation
      final revenue = baseRevenue + (i * 25000) + (i * i * 5000);
      
      chartData.add({
        'day': dayName,
        'revenue': revenue.toDouble(),
        'fullDate': DateFormat('dd/MM').format(date),
      });
    }
  ...
  List<Map<String, dynamic>> _getSampleRecentActivities() {
    return [
      {
      ...
      },
      {
        ...
        'description': '₹25,000 from ABC Farms',
        'time': '1 hour ago',
      ...
      },
      {
        ...
        'description': 'New employee Rajesh Kumar',
        'time': '2 hours ago',
  ...
  void _setupRealtimeListeners() {
    ...
    // Note: You need to implement these streams in SupabaseService
...
//does not work with chart and top selling
