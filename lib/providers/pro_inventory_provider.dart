import 'package:flutter/foundation.dart';
import 'package:supabase_flutter/supabase_flutter.dart';

class InventoryItem {
  final String id;
  final String name;
  final String? nameHindi;
  final double weightPerBag; // Weight per bag in kg
  final String unit;
  final double pricePerBag; // Price per bag
  double bags; // Number of bags in stock
  final double minBagsStock; // Minimum bags in stock
  final String category;
  final String? description;
  final bool isActive;
  final DateTime createdAt;
  final DateTime updatedAt;

  InventoryItem({
    required this.id,
    required this.name,
    this.nameHindi,
    required this.weightPerBag,
    this.unit = 'kg',
    required this.pricePerBag,
    required this.bags,
    required this.minBagsStock,
    this.category = 'Animal Feed',
    this.description,
    this.isActive = true,
    required this.createdAt,
    required this.updatedAt,
  });

  factory InventoryItem.fromMap(Map<String, dynamic> map) {
    try {
      // SAFE: Parse id with null check
      final id = map['id']?.toString() ?? '';
      if (id.isEmpty) {
        throw Exception('Inventory item missing ID');
      }

      // SAFE: Parse name with null check
      final name = map['name']?.toString() ?? 'Unknown Product';

      // SAFE: Parse nullable fields
      final nameHindi = map['name_hindi']?.toString();

      // SAFE: Parse numeric fields with null/default values
      final weightPerBag = _parseDouble(map['weight_per_bag']) ?? 50.0;
      final pricePerBag = _parseDouble(map['price_per_bag']) ?? 0.0;
      final bags = _parseDouble(map['bags']) ?? 0.0;
      final minBagsStock = _parseDouble(map['min_bags_stock']) ?? 100.0;

      // SAFE: Parse string fields with defaults
      final unit = map['unit']?.toString() ?? 'kg';
      final category = map['category']?.toString() ?? 'Animal Feed';
      final description = map['description']?.toString();

      // SAFE: Parse boolean with default
      final isActive = map['is_active']?.toString().toLowerCase() == 'true' || 
                      map['is_active'] == true || 
                      map['is_active'] == 1;

      // SAFE: Parse dates with fallbacks
      DateTime parseDate(String? dateString) {
        if (dateString == null) return DateTime.now();
        try {
          return DateTime.parse(dateString);
        } catch (_) {
          return DateTime.now();
        }
      }

      final createdAt = parseDate(map['created_at']?.toString());
      final updatedAt = parseDate(map['updated_at']?.toString());

      return InventoryItem(
        id: id,
        name: name,
        nameHindi: nameHindi,
        weightPerBag: weightPerBag,
        unit: unit,
        pricePerBag: pricePerBag,
        bags: bags,
        minBagsStock: minBagsStock,
        category: category,
        description: description,
        isActive: isActive,
        createdAt: createdAt,
        updatedAt: updatedAt,
      );
    } catch (e, stackTrace) {
      print('ERROR creating InventoryItem: $e');
      print('Stack trace: $stackTrace');
      print('Problematic data: $map');
      
      // Return a safe default item to prevent crashes
      return InventoryItem(
        id: map['id']?.toString() ?? 'error_${DateTime.now().millisecondsSinceEpoch}',
        name: 'Error Loading Product',
        weightPerBag: 50.0,
        unit: 'kg',
        pricePerBag: 0.0,
        bags: 0.0,
        minBagsStock: 100.0,
        category: 'Error',
        description: 'This product failed to load properly',
        isActive: false,
        createdAt: DateTime.now(),
        updatedAt: DateTime.now(),
      );
    }
  }

  // Helper method to safely parse doubles
  static double? _parseDouble(dynamic value) {
    if (value == null) return null;
    
    if (value is num) {
      return value.toDouble();
    } else if (value is String) {
      return double.tryParse(value);
    }
    return null;
  }

  Map<String, dynamic> toMap() {
    return {
      'name': name,
      'name_hindi': nameHindi,
      'weight_per_bag': weightPerBag,
      'unit': unit,
      'price_per_bag': pricePerBag,
      'bags': bags,
      'min_bags_stock': minBagsStock,
      'category': category,
      'description': description,
      'is_active': isActive,
    };
  }

  // Calculated properties with null safety
  double get tons {
    try {
      return (bags * weightPerBag) / 1000; // Convert kg to tons
    } catch (e) {
      print('Error calculating tons: $e');
      return 0.0;
    }
  }
  
  double get stock {
    try {
      return bags * weightPerBag; // Total weight in kg
    } catch (e) {
      print('Error calculating stock: $e');
      return 0.0;
    }
  }
  
  double get pricePerTon {
    try {
      if (weightPerBag > 0) {
        return (pricePerBag * 1000) / weightPerBag; // Price per ton
      }
      return 0.0;
    } catch (e) {
      print('Error calculating pricePerTon: $e');
      return 0.0;
    }
  }
  
  bool get isLowStock {
    try {
      return bags <= minBagsStock;
    } catch (e) {
      print('Error checking low stock: $e');
      return false;
    }
  }
  
  double get totalValue {
    try {
      return bags * pricePerBag;
    } catch (e) {
      print('Error calculating total value: $e');
      return 0.0;
    }
  }
  
  // Legacy properties for backward compatibility
  double get weight => weightPerBag;
  double get price => pricePerBag;
  double get minStock => minBagsStock;
  
  String get displayName {
    if (nameHindi != null && nameHindi!.isNotEmpty) {
      return '$name / $nameHindi';
    }
    return name;
  }
}

class InventoryProvider extends ChangeNotifier {
  final SupabaseClient _supabase = Supabase.instance.client;
  
  List<InventoryItem> _inventoryItems = [];
  bool _isLoading = false;
  String? _error;

  List<InventoryItem> get inventoryItems => List.from(_inventoryItems);
  bool get isLoading => _isLoading;
  String? get error => _error;

  // Filtered items
  List<InventoryItem> get activeItems => 
      _inventoryItems.where((item) => item.isActive).toList();
  
  List<InventoryItem> get lowStockItems => 
      _inventoryItems.where((item) => item.isLowStock && item.isActive).toList();

  // Initialize provider
  InventoryProvider() {
    // Initialize with delay to ensure context is ready
    Future.delayed(const Duration(milliseconds: 100), () {
      _loadInventory();
      _setupRealtimeSubscription();
    });
  }

  // Load inventory from Supabase with error handling
  Future<void> _loadInventory() async {
    try {
      if (_isLoading) return;
      
      _isLoading = true;
      _error = null;
      notifyListeners();

      print('Loading inventory from Supabase...');
      
      final response = await _supabase
          .from('production_products')
          .select('*')
          .order('name', ascending: true)
          .timeout(const Duration(seconds: 10));

      if (response != null && response is List) {
        print('Received ${response.length} inventory items');
        
        // Clear existing items
        _inventoryItems.clear();
        
        // Process each item safely
        int successCount = 0;
        int errorCount = 0;
        
        for (var itemData in response) {
          try {
            final item = InventoryItem.fromMap(itemData);
            if (item.id.isNotEmpty && !item.id.startsWith('error_')) {
              _inventoryItems.add(item);
              successCount++;
            } else {
              errorCount++;
              print('Skipping invalid item: ${itemData['id']}');
            }
          } catch (e) {
            errorCount++;
            print('Error processing inventory item: $e');
            print('Item data: $itemData');
          }
        }
        
        print('Inventory loaded: $successCount items, $errorCount errors');
        
        if (errorCount > 0 && successCount == 0) {
          _error = 'Failed to load any inventory items. Please check database connection.';
        }
      } else {
        print('No data or invalid response from Supabase');
        _error = 'No inventory data available';
      }
    } catch (e, stackTrace) {
      print('Error loading inventory: $e');
      print('Stack trace: $stackTrace');
      
      // User-friendly error messages
      if (e.toString().contains('connection') || 
          e.toString().contains('network') || 
          e.toString().contains('timeout')) {
        _error = 'Network error. Please check your internet connection.';
      } else if (e.toString().contains('auth') || 
                 e.toString().contains('permission')) {
        _error = 'Authentication error. Please log in again.';
      } else {
        _error = 'Failed to load inventory: ${e.toString().split(':').first}';
      }
      
      _inventoryItems = []; // Clear to prevent showing corrupt data
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  // Set up realtime subscription with error handling
  void _setupRealtimeSubscription() {
    try {
      _supabase
          .from('production_products')
          .stream(primaryKey: ['id'])
          .listen(
            (List<Map<String, dynamic>> updates) {
              _handleRealtimeUpdates(updates);
            },
            onError: (error) {
              print('Realtime subscription error: $error');
            },
            cancelOnError: false,
          );
    } catch (e) {
      print('Failed to setup realtime subscription: $e');
    }
  }

  void _handleRealtimeUpdates(List<Map<String, dynamic>> updates) {
    try {
      for (final update in updates) {
        final id = update['id']?.toString();
        if (id == null) continue;
        
        final index = _inventoryItems.indexWhere((item) => item.id == id);
        
        if (index != -1) {
          // Check if item is being deleted (is_active = false)
          final isActive = update['is_active']?.toString().toLowerCase() == 'true' || 
                          update['is_active'] == true || 
                          update['is_active'] == 1;
          
          if (!isActive) {
            // Remove deleted item
            _inventoryItems.removeAt(index);
          } else {
            // Update existing item
            try {
              _inventoryItems[index] = InventoryItem.fromMap(update);
            } catch (e) {
              print('Error updating item from realtime: $e');
            }
          }
        } else {
          // Add new item
          try {
            final newItem = InventoryItem.fromMap(update);
            if (newItem.isActive && !newItem.id.startsWith('error_')) {
              _inventoryItems.add(newItem);
            }
          } catch (e) {
            print('Error adding new item from realtime: $e');
          }
        }
      }
      notifyListeners();
    } catch (e) {
      print('Error handling realtime updates: $e');
    }
  }

  // Add new product
  Future<void> addProduct(InventoryItem product) async {
    try {
      _isLoading = true;
      _error = null;
      notifyListeners();

      final user = _supabase.auth.currentUser;
      
      final data = {
        ...product.toMap(),
        'created_by': user?.id,
        'created_at': DateTime.now().toIso8601String(),
        'updated_at': DateTime.now().toIso8601String(),
      };

      final response = await _supabase
          .from('production_products')
          .insert(data)
          .select()
          .single()
          .timeout(const Duration(seconds: 10));

      if (response != null && response.isNotEmpty) {
        // Add log
        try {
          await _supabase.from('inventory_logs').insert({
            'product_id': response['id'],
            'action': 'add',
            'new_value': product.toMap(),
            'performed_by': user?.id,
            'timestamp': DateTime.now().toIso8601String(),
          });
        } catch (e) {
          print('Error adding inventory log: $e');
        }

        await _loadInventory(); // Reload to get the new item
      }
    } catch (e, stackTrace) {
      print('Error adding product: $e');
      print('Stack trace: $stackTrace');
      _error = 'Failed to add product: ${e.toString().split(':').first}';
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  // Update product
  Future<void> updateProduct(String id, InventoryItem updatedProduct) async {
    try {
      _isLoading = true;
      _error = null;
      notifyListeners();

      final user = _supabase.auth.currentUser;
      
      // Find old product safely
      InventoryItem? oldProduct;
      try {
        oldProduct = _inventoryItems.firstWhere((item) => item.id == id);
      } catch (_) {
        oldProduct = null;
      }

      final data = {
        ...updatedProduct.toMap(),
        'updated_at': DateTime.now().toIso8601String(),
      };

      await _supabase
          .from('production_products')
          .update(data)
          .eq('id', id)
          .timeout(const Duration(seconds: 10));

      // Add log
      try {
        await _supabase.from('inventory_logs').insert({
          'product_id': id,
          'action': 'update',
          'old_value': oldProduct?.toMap(),
          'new_value': updatedProduct.toMap(),
          'performed_by': user?.id,
          'timestamp': DateTime.now().toIso8601String(),
        });
      } catch (e) {
        print('Error adding update log: $e');
      }

      await _loadInventory();
    } catch (e, stackTrace) {
      print('Error updating product: $e');
      print('Stack trace: $stackTrace');
      _error = 'Failed to update product: ${e.toString().split(':').first}';
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  // Delete product (soft delete by setting isActive to false)
  Future<void> deleteProduct(String id) async {
    try {
      _isLoading = true;
      _error = null;
      notifyListeners();

      final user = _supabase.auth.currentUser;
      
      // Find product safely
      InventoryItem? product;
      try {
        product = _inventoryItems.firstWhere((item) => item.id == id);
      } catch (_) {
        product = null;
      }

      await _supabase
          .from('production_products')
          .update({
            'is_active': false,
            'updated_at': DateTime.now().toIso8601String(),
          })
          .eq('id', id)
          .timeout(const Duration(seconds: 10));

      // Add log
      try {
        await _supabase.from('inventory_logs').insert({
          'product_id': id,
          'action': 'delete',
          'old_value': product?.toMap(),
          'performed_by': user?.id,
          'timestamp': DateTime.now().toIso8601String(),
        });
      } catch (e) {
        print('Error adding delete log: $e');
      }

      await _loadInventory();
    } catch (e, stackTrace) {
      print('Error deleting product: $e');
      print('Stack trace: $stackTrace');
      _error = 'Failed to delete product: ${e.toString().split(':').first}';
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  // Update stock by bags
  Future<void> updateStock(String id, double newBags, {String? notes}) async {
    try {
      _isLoading = true;
      _error = null;
      notifyListeners();

      final user = _supabase.auth.currentUser;
      
      // Find old product safely
      InventoryItem? oldProduct;
      try {
        oldProduct = _inventoryItems.firstWhere((item) => item.id == id);
      } catch (_) {
        oldProduct = null;
      }
      
      final quantityChange = oldProduct != null ? newBags - oldProduct.bags : newBags;

      await _supabase
          .from('production_products')
          .update({
            'bags': newBags,
            'updated_at': DateTime.now().toIso8601String(),
          })
          .eq('id', id)
          .timeout(const Duration(seconds: 10));

      // Add log
      try {
        await _supabase.from('inventory_logs').insert({
          'product_id': id,
          'action': 'stock_update',
          'old_value': oldProduct != null ? {'bags': oldProduct.bags} : null,
          'new_value': {'bags': newBags},
          'quantity_change': quantityChange,
          'notes': notes,
          'performed_by': user?.id,
          'timestamp': DateTime.now().toIso8601String(),
        });
      } catch (e) {
        print('Error adding stock update log: $e');
      }

      await _loadInventory();
    } catch (e, stackTrace) {
      print('Error updating stock: $e');
      print('Stack trace: $stackTrace');
      _error = 'Failed to update stock: ${e.toString().split(':').first}';
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  // Search products
  List<InventoryItem> searchProducts(String query) {
    if (query.isEmpty) return List.from(activeItems);
    
    final lowercaseQuery = query.toLowerCase();
    return activeItems.where((item) {
      try {
        return item.name.toLowerCase().contains(lowercaseQuery) ||
               (item.nameHindi?.toLowerCase() ?? '').contains(lowercaseQuery) ||
               item.category.toLowerCase().contains(lowercaseQuery) ||
               (item.description?.toLowerCase() ?? '').contains(lowercaseQuery);
      } catch (e) {
        print('Error in search for item ${item.id}: $e');
        return false;
      }
    }).toList();
  }

  // Get inventory summary
  Map<String, dynamic> getInventorySummary() {
    try {
      final activeProducts = activeItems;
      final lowStock = lowStockItems.length;
      final totalProducts = activeProducts.length;
      
      double totalBags = 0.0;
      double totalTons = 0.0;
      double totalValue = 0.0;
      
      for (var item in activeProducts) {
        try {
          totalBags += item.bags;
          totalTons += item.tons;
          totalValue += item.totalValue;
        } catch (e) {
          print('Error calculating summary for item ${item.id}: $e');
        }
      }
      
      return {
        'totalProducts': totalProducts,
        'lowStockItems': lowStock,
        'totalBags': totalBags,
        'totalTons': totalTons,
        'totalValue': totalValue,
      };
    } catch (e) {
      print('Error getting inventory summary: $e');
      return {
        'totalProducts': 0,
        'lowStockItems': 0,
        'totalBags': 0.0,
        'totalTons': 0.0,
        'totalValue': 0.0,
      };
    }
  }

  // Refresh data
  Future<void> refresh() async {
    await _loadInventory();
  }

  // Clear error
  void clearError() {
    _error = null;
    notifyListeners();
  }

  // Find item by ID
  InventoryItem? findById(String id) {
    try {
      return _inventoryItems.firstWhere((item) => item.id == id);
    } catch (_) {
      return null;
    }
  }
}

















// import 'package:flutter/foundation.dart';
// import 'package:supabase_flutter/supabase_flutter.dart';

// class InventoryItem {
//   final String id;
//   final String name;
//   final String? nameHindi;
//   final double weightPerBag; // Weight per bag in kg
//   final String unit;
//   final double pricePerBag; // Price per bag
//   double bags; // Number of bags in stock
//   final double minBagsStock; // Minimum bags in stock
//   final String category;
//   final String? description;
//   final bool isActive;
//   final DateTime createdAt;
//   final DateTime updatedAt;

//   InventoryItem({
//     required this.id,
//     required this.name,
//     this.nameHindi,
//     required this.weightPerBag,
//     this.unit = 'kg',
//     required this.pricePerBag,
//     required this.bags,
//     required this.minBagsStock,
//     this.category = 'Animal Feed',
//     this.description,
//     this.isActive = true,
//     required this.createdAt,
//     required this.updatedAt,
//   });

//   factory InventoryItem.fromMap(Map<String, dynamic> map) {
//     return InventoryItem(
//       id: map['id'] as String,
//       name: map['name'] as String,
//       nameHindi: map['name_hindi'] as String?,
//       weightPerBag: (map['weight_per_bag'] as num).toDouble(),
//       unit: map['unit'] as String? ?? 'kg',
//       pricePerBag: (map['price_per_bag'] as num).toDouble(),
//       bags: (map['bags'] as num).toDouble(),
//       minBagsStock: (map['min_bags_stock'] as num).toDouble(),
//       category: map['category'] as String? ?? 'Animal Feed',
//       description: map['description'] as String?,
//       isActive: map['is_active'] as bool? ?? true,
//       createdAt: DateTime.parse(map['created_at'] as String),
//       updatedAt: DateTime.parse(map['updated_at'] as String),
//     );
//   }

//   Map<String, dynamic> toMap() {
//     return {
//       'name': name,
//       'name_hindi': nameHindi,
//       'weight_per_bag': weightPerBag,
//       'unit': unit,
//       'price_per_bag': pricePerBag,
//       'bags': bags,
//       'min_bags_stock': minBagsStock,
//       'category': category,
//       'description': description,
//       'is_active': isActive,
//     };
//   }

//   // Calculated properties
//   double get tons => (bags * weightPerBag) / 1000; // Convert kg to tons
//   double get stock => bags * weightPerBag; // Total weight in kg
//   double get pricePerTon => (pricePerBag * 1000) / weightPerBag; // Price per ton
//   bool get isLowStock => bags <= minBagsStock;
//   double get totalValue => bags * pricePerBag;
  
//   // Legacy properties for backward compatibility
//   double get weight => weightPerBag;
//   double get price => pricePerBag;
//   double get minStock => minBagsStock;
//   String get displayName => nameHindi != null ? '$name / $nameHindi' : name;
// }

// class InventoryProvider extends ChangeNotifier {
//   final SupabaseClient _supabase = Supabase.instance.client;
  
//   List<InventoryItem> _inventoryItems = [];
//   bool _isLoading = false;
//   String? _error;

//   List<InventoryItem> get inventoryItems => _inventoryItems;
//   bool get isLoading => _isLoading;
//   String? get error => _error;

//   // Filtered items
//   List<InventoryItem> get activeItems => 
//       _inventoryItems.where((item) => item.isActive).toList();
  
//   List<InventoryItem> get lowStockItems => 
//       _inventoryItems.where((item) => item.isLowStock && item.isActive).toList();

//   // Initialize provider
//   InventoryProvider() {
//     _loadInventory();
//     _setupRealtimeSubscription();
//   }

//   // Load inventory from Supabase
//   Future<void> _loadInventory() async {
//     try {
//       _isLoading = true;
//       _error = null;
//       notifyListeners();

//       final response = await _supabase
//           .from('production_products')
//           .select('*')
//           .order('name', ascending: true);

//       _inventoryItems = (response as List)
//           .map((item) => InventoryItem.fromMap(item))
//           .toList();
//         } catch (e) {
//       _error = 'Failed to load inventory: $e';
//       print('Error loading inventory: $e');
//     } finally {
//       _isLoading = false;
//       notifyListeners();
//     }
//   }

//   // Set up realtime subscription
//   void _setupRealtimeSubscription() {
//     _supabase
//         .from('production_products')
//         .stream(primaryKey: ['id'])
//         .listen((List<Map<String, dynamic>> updates) {
//           for (final update in updates) {
//             final index = _inventoryItems.indexWhere((item) => item.id == update['id']);
            
//             if (index != -1) {
//               _inventoryItems[index] = InventoryItem.fromMap(update);
//             } else {
//               _inventoryItems.add(InventoryItem.fromMap(update));
//             }
//           }
//           notifyListeners();
//         });
//   }

//   // Add new product
//   Future<void> addProduct(InventoryItem product) async {
//     try {
//       _isLoading = true;
//       notifyListeners();

//       final user = _supabase.auth.currentUser;
      
//       final response = await _supabase
//           .from('production_products')
//           .insert({
//             ...product.toMap(),
//             'created_by': user?.id,
//           })
//           .select();

//       if (response.isNotEmpty) {
//         // Add log
//         await _supabase.from('inventory_logs').insert({
//           'product_id': response[0]['id'],
//           'action': 'add',
//           'new_value': product.toMap(),
//           'performed_by': user?.id,
//         });

//         await _loadInventory(); // Reload to get the new item
//       }
//     } catch (e) {
//       _error = 'Failed to add product: $e';
//       print('Error adding product: $e');
//       rethrow;
//     } finally {
//       _isLoading = false;
//       notifyListeners();
//     }
//   }

//   // Update product
//   Future<void> updateProduct(String id, InventoryItem updatedProduct) async {
//     try {
//       _isLoading = true;
//       notifyListeners();

//       final user = _supabase.auth.currentUser;
//       final oldProduct = _inventoryItems.firstWhere((item) => item.id == id);

//       await _supabase
//           .from('production_products')
//           .update(updatedProduct.toMap())
//           .eq('id', id);

//       // Add log
//       await _supabase.from('inventory_logs').insert({
//         'product_id': id,
//         'action': 'update',
//         'old_value': oldProduct.toMap(),
//         'new_value': updatedProduct.toMap(),
//         'performed_by': user?.id,
//       });

//       await _loadInventory();
//     } catch (e) {
//       _error = 'Failed to update product: $e';
//       print('Error updating product: $e');
//       rethrow;
//     } finally {
//       _isLoading = false;
//       notifyListeners();
//     }
//   }

//   // Delete product (soft delete by setting isActive to false)
//   Future<void> deleteProduct(String id) async {
//     try {
//       _isLoading = true;
//       notifyListeners();

//       final user = _supabase.auth.currentUser;
//       final product = _inventoryItems.firstWhere((item) => item.id == id);

//       await _supabase
//           .from('production_products')
//           .update({'is_active': false})
//           .eq('id', id);

//       // Add log
//       await _supabase.from('inventory_logs').insert({
//         'product_id': id,
//         'action': 'delete',
//         'old_value': product.toMap(),
//         'performed_by': user?.id,
//       });

//       await _loadInventory();
//     } catch (e) {
//       _error = 'Failed to delete product: $e';
//       print('Error deleting product: $e');
//       rethrow;
//     } finally {
//       _isLoading = false;
//       notifyListeners();
//     }
//   }

//   // Update stock by bags
//   Future<void> updateStock(String id, double newBags, {String? notes}) async {
//     try {
//       _isLoading = true;
//       notifyListeners();

//       final user = _supabase.auth.currentUser;
//       final oldProduct = _inventoryItems.firstWhere((item) => item.id == id);
//       final quantityChange = newBags - oldProduct.bags;

//       await _supabase
//           .from('production_products')
//           .update({'bags': newBags})
//           .eq('id', id);

//       // Add log
//       await _supabase.from('inventory_logs').insert({
//         'product_id': id,
//         'action': 'stock_update',
//         'old_value': {'bags': oldProduct.bags},
//         'new_value': {'bags': newBags},
//         'quantity_change': quantityChange,
//         'performed_by': user?.id,
//       });

//       await _loadInventory();
//     } catch (e) {
//       _error = 'Failed to update stock: $e';
//       print('Error updating stock: $e');
//       rethrow;
//     } finally {
//       _isLoading = false;
//       notifyListeners();
//     }
//   }

//   // Search products
//   List<InventoryItem> searchProducts(String query) {
//     if (query.isEmpty) return activeItems;
    
//     return activeItems.where((item) {
//       final name = item.name.toLowerCase();
//       final hindiName = item.nameHindi?.toLowerCase() ?? '';
//       final searchQuery = query.toLowerCase();
      
//       return name.contains(searchQuery) || 
//              hindiName.contains(searchQuery) ||
//              item.category.toLowerCase().contains(searchQuery);
//     }).toList();
//   }

//   // Get inventory summary
//   Map<String, dynamic> getInventorySummary() {
//     final activeProducts = activeItems;
//     final lowStock = lowStockItems.length;
//     final totalProducts = activeProducts.length;
//     final totalBags = activeProducts.fold(0.0, (sum, item) => sum + item.bags);
//     final totalTons = activeProducts.fold(0.0, (sum, item) => sum + item.tons);
//     final totalValue = activeProducts.fold(0.0, (sum, item) => sum + item.totalValue);
    
//     return {
//       'totalProducts': totalProducts,
//       'lowStockItems': lowStock,
//       'totalBags': totalBags,
//       'totalTons': totalTons,
//       'totalValue': totalValue,
//     };
//   }

//   // Refresh data
//   Future<void> refresh() async {
//     await _loadInventory();
//   }
// }