...
// Dashboard Content Widget with Real-time Data
...
class _DashboardContentState extends State<DashboardContent> with WidgetsBindingObserver {
  ...
  final Map<String, List<String>> _districtTalukaData = {
    ...
    "Bhandara": ["Bhandara", "Tumsar", "Sakoli", "Mohadi", "Pauni"],
    "Buldhana": ["Buldhana", "Chikhli", "Deulgaon Raja", "Khamgaon", "Mehkar", "Nandura", "Malkapur", "Jalgaon Jamod", "Sindkhed Raja"],
    "Chandrapur": ["Chandrapur", "Ballarpur", "Bhadravati", "Chimur", "Nagbhid", "Rajura", "Warora"],
    "Dhule": ["Dhule", "Shirpur", "Sakri", "Sindkheda", "Dondaicha"],
    "Gadchiroli": ["Gadchiroli", "Aheri", "Chamorshi", "Etapalli", "Rajura", "Armori"],
    "Gondiya": ["Gondiya", "Amgaon", "Deori", "Salekasa", "Tirora"],
    "Hingoli": ["Hingoli", "Kalamnuri", "Basmath", "Sengaon"],
    "Jalgaon": ["Jalgaon", "Amalner", "Chopda", "Dharangaon", "Erandol", "Pachora", "Parola", "Bhusawal", "Raver"],
    "Jalna": ["Jalna", "Bhokardan", "Badnapur", "Partur", "Ghansawangi", "Ambad"],
    "Kolhapur": ["Karvir", "Panhala", "Shirol", "Hatkanangale", "Kagal", "Shahuwadi", "Ajara", "Gadhinglaj", "Chandgad", "Radhanagari", "Jat", "Bhudargad"],
    "Latur": ["Latur", "Ausa", "Ahmedpur", "Udgir", "Nilanga", "Renapur", "Chakur"],
    "Mumbai City": ["Mumbai City"],
    "Mumbai Suburban": ["Andheri", "Borivali", "Kurla", "Mulund", "Bandra"],
    "Nagpur": ["Nagpur", "Hingna", "Parseoni", "Kalmeshwar", "Umred", "Kuhi", "Savner"],
    "Nanded": ["Nanded", "Deglur", "Biloli", "Bhokar", "Mukhed", "Loha", "Ardhapur", "Umri"],
    "Nandurbar": ["Nandurbar", "Nawapur", "Shahada", "Taloda", "Akkalkuwa"],
    "Nashik": ["Nashik", "Dindori", "Igatpuri", "Niphad", "Sinnar", "Yeola", "Trimbakeshwar", "Baglan", "Chandwad"],
    "Osmanabad": ["Osmanabad", "Tuljapur", "Paranda", "Lohara", "Ausa", "Kalamb"],
    "Palghar": ["Palghar", "Dahanu", "Talasari", "Umbergaon", "Vikramgad", "Jawhar", "Mokhada"],
    "Parbhani": ["Parbhani", "Gangakhed", "Purna", "Selu", "Pathri"],
    "Pune": ["Pune", "Haveli", "Maval", "Mulshi", "Baramati", "Shirur", "Daund", "Indapur", "Junnar", "Khed", "Bhor"],
    "Raigad": ["Alibag", "Karjat", "Khalapur", "Mahad", "Mangaon", "Mhasala", "Panvel", "Pen", "Roha"],
    "Ratnagiri": ["Ratnagiri", "Chiplun", "Khed", "Guhagar", "Lanja", "Sangameshwar"],
    "Sangli": ["Sangli", "Miraj", "Tasgaon", "Jat", "Kavathe Mahankal", "Palus"],
    "Satara": ["Satara", "Karad", "Wai", "Khandala", "Patan", "Phaltan", "Man"],
    "Sindhudurg": ["Sindhudurg", "Kankavli", "Malvan", "Vengurla", "Sawantwadi"],
    "Solapur": ["Solapur", "Akkalkot", "Barshi", "Mangalwedha", "Pandharpur", "Madha", "Karmala", "Sangola"],
    "Thane": ["Thane", "Bhiwandi", "Kalyan", "Ulhasnagar", "Ambarnath", "Shahapur", "Murbad", "Wada", "Jawahar"],
    "Wardha": ["Wardha", "Deoli", "Arvi"],
    "Washim": ["Washim", "Mangrulpir", "Karanja"],
    "Yavatmal": ["Yavatmal", "Umarkhed", "Darwha", "Pusad", "Ghatanji", "Kalamb"],
  };
  
  // Sales data storage
  Map<String, double> _talukaSalesData = {};
  ...
  // Employee list
  List<Map<String, dynamic>> _districtEmployees = [];
// employee_id -> taluka
  ...
  Future<void> _initializeDashboard() async {
    try {
      ...
      if (district == null || district.isEmpty) {
        print('❌ No district assigned to manager');
        setState(() {
          _isLoadingTarget = false;
          _isLoadingSales = false;
        });
  ...
  Future<void> _loadDistrictEmployees() async {
    try {
      print('📋 Loading employees for district: $_selectedDistrict');
      
      final employees = await supabase
          .from('emp_profile')
          .select('id, emp_id, full_name, district, role, position')
          .eq('district', _selectedDistrict);
      
      setState(() {
        _districtEmployees = List<Map<String, dynamic>>.from(employees);
      });
      
      print('✅ Found ${_districtEmployees.length} employees');
      
    } catch (e) {
      print('❌ Error loading district employees: $e');
      _districtEmployees = [];
    }
  }
  ...
  void _setupRealtimeSubscriptions() {
    try {
      _ordersChannel = supabase.channel('manager-orders-${_selectedDistrict}')
          .onPostgresChanges(
          ...
          .subscribe((status, [error]) {
            if (status == 'SUBSCRIBED') {
              print('✅ Orders channel subscribed');
      ...
      if (_managerId != null) {
        _targetChannel = supabase.channel('manager-targets-${_managerId}')
            .onPostgresChanges(
              event: PostgresChangeEvent.all,
              schema: 'public',
              table: 'own_marketing_targets',
              callback: (payload) {
                if (payload.newRecord['manager_id'] == _managerId) {
                  _debouncedTargetRefresh();
                }
              },
            )
            .subscribe((status, [error]) {
              if (status == 'SUBSCRIBED') {
                print('✅ Target channel subscribed');
              }
            });
      }
  ...
  Future<void> _loadSalesData() async {
    try {
      ...
      // Get all talukas for this district from the mapping
      final districtTalukas = _districtTalukaData[_selectedDistrict] ?? [];
      print('📌 Talukas in $_selectedDistrict: ${districtTalukas.length}');
      
      // Initialize all talukas with zero sales
      _talukaSalesData = {};
      for (var taluka in districtTalukas) {
        _talukaSalesData[taluka] = 0.0;
      }
      ...
      // Fetch ALL completed orders for this district
      ...
      print('📦 Found ${allOrders.length} completed orders');
      
      // Store recent orders (last 3)
      _recentOrders = List<Map<String, dynamic>>.from(allOrders.take(3));
      
      // Process each order - need to map orders to talukas
      // Since orders don't have taluka field, we need to get employee's taluka from profile
      // But employees also don't have taluka in the table
      // For now, distribute orders evenly among talukas or use a default distribution
      // This is a temporary solution - you'll need to add taluka field to either orders or employees
      
      if (allOrders.isNotEmpty && districtTalukas.isNotEmpty) {
        // Distribute orders proportionally based on employee count per taluka
        // For now, just show sample data to demonstrate the chart
        for (int i = 0; i < allOrders.length; i++) {
          ...
          // Add bags
          final bagsValue = order['bags'];
          if (bagsValue != null) {
            if (bagsValue is int) {
              _totalBags += bagsValue;
            } else if (bagsValue is num) {
              _totalBags += bagsValue.toInt();
            }
          }
          ...
          // Distribute order to a taluka (for demo - shows all talukas with varying data)
          final talukaIndex = i % districtTalukas.length;
          final taluka = districtTalukas[talukaIndex];
          _talukaSalesData[taluka] = (_talukaSalesData[taluka] ?? 0) + weightInTons;
        }
      } else if (allOrders.isEmpty) {
        // If no orders, still show talukas with zero values to demonstrate the chart
        for (var taluka in districtTalukas) {
          _talukaSalesData[taluka] = 0.0;
        }
      }
      
      // Prepare chart data (sort by sales)
      _chartData = _talukaSalesData.entries
          .map((e) => {'taluka': e.key, 'sales': e.value})
          .toList()
        ..sort((a, b) => (b['sales'] as double).compareTo(a['sales'] as double));
      
      print('💰 Revenue: ₹${_totalRevenue.toStringAsFixed(2)}');
      print('📊 Orders: $_totalOrders');
      print('📦 Bags: $_totalBags');
      print('⚖️ Weight: ${_totalDistrictSales.toStringAsFixed(2)} T');
      print('📈 Taluka Performance:');
      for (var taluka in _chartData) {
        print('   ${taluka['taluka']}: ${(taluka['sales'] as double).toStringAsFixed(2)} T');
      }
      
      // Update target
      if (_currentTargetData != null) {
        setState(() {
          _currentTargetData!['achieved_revenue'] = _totalRevenue;
          _currentTargetData!['achieved_orders'] = _totalOrders.toDouble();
        });
      }
  ...
  double _calculateWeightInTons(Map<String, dynamic> order) {
    ...
    final bags = (order['bags'] as num?)?.toInt() ?? 0;
    return (bags * 50) / 1000;
  }
  ...
  Future<void> _loadCurrentTarget() async {
    try {
      ...
      if (targetData != null) {
        if (!targetData.containsKey('revenue_target')) targetData['revenue_target'] = 0.0;
        if (!targetData.containsKey('order_target')) targetData['order_target'] = 0.0;
        if (!targetData.containsKey('achieved_revenue')) targetData['achieved_revenue'] = _totalRevenue;
        if (!targetData.containsKey('achieved_orders')) targetData['achieved_orders'] = _totalOrders.toDouble();
      }
    ...
    try {
      ...
      if (!silent && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ Updated: $_totalOrders orders, ₹${_totalRevenue.toStringAsFixed(0)} revenue'),
  ...
  // Chart Widget - Shows taluka-wise sales
  Widget _buildChart() {
    if (_chartData.isEmpty || _isLoadingSales) {
      return SizedBox(
        height: 250,
        child: Center(
          child: Column(
    ...
    final chartWidth = _chartData.length * 80.0;
    final maxY = _getMaxY();
                      ...
                      lineBarsData: [
                        LineChartBarData(
                          spots: List.generate(_chartData.length, (i) => 
                            FlSpot(i.toDouble(), _chartData[i]['sales'] as double)
                          ),
                          isCurved: true,
                          color: themePrimary,
                          barWidth: 3,
                        ...
                        touchTooltipData: LineTouchTooltipData(
                          ...
                          getTooltipItems: (List<LineBarSpot> touchedSpots) {
                            return touchedSpots.map((spot) {
                              final taluka = _chartData[spot.x.toInt()];
                              return LineTooltipItem(
                                '${taluka['taluka']}\n${(taluka['sales'] as double).toStringAsFixed(2)} T',
                                const TextStyle(
                                  color: Colors.white,
                                  fontSize: 12,
                                ),
  ...
  double _getMaxY() {
    ...
    final maxSales = _chartData
        .map((e) => e['sales'] as double)
        .reduce((a, b) => a > b ? a : b);
    
    if (maxSales == 0) return 10.0;
      ...
      return Container(
        ...
        child: const Row(
          children: [
            ...
            SizedBox(width: 8),
            Text('No recent orders', style: TextStyle(fontSize: 12, color: Colors.grey)),
          ],
        ),
      );
    }
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${order['bags']} bags • ₹${order['total_price']}',
                  ...
                  Text(
                    DateFormat('dd MMM').format(DateTime.parse(order['created_at'])),
        ...
        Row(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(colors: [Colors.orange.withOpacity(0.8), Colors.orange.withOpacity(0.6)]),
                borderRadius: BorderRadius.circular(20),
              ),
              child: const Row(
                children: [
                  ...
                  SizedBox(width: 4),
                  Text("Not Assigned", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 10)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildTargetState(Map<String, dynamic> targetData) {
    ...
    final achievedRevenue = (targetData['achieved_revenue'] ?? _totalRevenue).toDouble();
    final achievedOrders = (targetData['achieved_orders'] ?? _totalOrders).toDouble();
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const Text("LIVE", style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Colors.green)),
                  ],
                ),
              ),
            ],
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                DateFormat('dd MMM, hh:mm a').format(_lastDataUpdate), 
          ...
          Row(
            ...
            children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text("Achieved", style: TextStyle(fontSize: 10, color: Colors.grey[600])),
                const SizedBox(height: 4),
                Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
                  ...
                  const SizedBox(width: 4),
                  Text("/ $prefix${target.toStringAsFixed(0)}", style: TextStyle(fontSize: 14, color: Colors.grey[600])),
                ]),
              ]),
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        "Welcome, ${_managerName ?? 'Manager'}",
                        style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.grey[800]),
                      ),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          Icon(Icons.location_on, size: 16, color: GlobalColors.primaryBlue),
                          const SizedBox(width: 8),
                          Text("District: $_selectedDistrict", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.grey[700])),
                        ],
                      ),
                      const SizedBox(height: 8),
                      Row(
                        ...
                        children: [
                          Text(
                            "Managing ${_districtEmployees.length} Employees",
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                const SizedBox(width: 4),
                                Text('Live updates', style: TextStyle(fontSize: 9, color: Colors.green[700], fontWeight: FontWeight.w500)),
                              ],
                            ),
                          ),
                        ],
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      ...
                      colors: [GlobalColors.primaryBlue, GlobalColors.primaryBlue.withOpacity(0.8)],
                    ),
                    borderRadius: BorderRadius.circular(16),
                    boxShadow: [BoxShadow(color: GlobalColors.primaryBlue.withOpacity(0.3), blurRadius: 15, offset: const Offset(0, 5))],
                  ),
                  child: Row(
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                Text("Total District Sales", style: GoogleFonts.poppins(color: Colors.white.withOpacity(0.9), fontSize: 14)),
                                if (_isLoadingSales)
                                  const SizedBox(width: 8, height: 12, child: CircularProgressIndicator(strokeWidth: 1, color: Colors.white)),
                              ],
                            ),
                            const SizedBox(height: 8),
                            Text("${_totalDistrictSales.toStringAsFixed(2)} T", style: GoogleFonts.poppins(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 8),
                            Text(
                              "From $_totalOrders orders • ${_districtEmployees.length} employees",
                            ...
                            Row(
                              children: [
                                Container(width: 8, height: 8, decoration: const BoxDecoration(color: Colors.green, shape: BoxShape.circle)),
                                const SizedBox(width: 6),
                                Text("Real-time from completed orders", style: GoogleFonts.poppins(color: Colors.white.withOpacity(0.8), fontSize: 10)),
                              ],
                            ),
                          ],
                        ),
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Column(
                          children: [
                            Text("Revenue", style: TextStyle(fontSize: 11, color: Colors.grey[600])),
                            const SizedBox(height: 2),
                            Text("₹${NumberFormat.compact().format(_totalRevenue)}", 
                        ...
                        child: Column(
                          children: [
                            ...
                            Text(_totalOrders.toString(), 
                              style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: GlobalColors.primaryBlue)),
                          ],
                        ),
                      ),
                      Container(width: 1, height: 30, color: Colors.grey.shade300),
                      Expanded(
                        child: Column(
                          children: [
                            Text("Bags", style: TextStyle(fontSize: 11, color: Colors.grey[600])),
                            const SizedBox(height: 2),
                            Text(NumberFormat.compact().format(_totalBags), 
                ...
                // Sales Trend Chart - Taluka-wise Performance
                Container(
                  width: double.infinity,
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
...
// // Dashboard Content Widget with Real-time Data
// // Dashboard Content Widget with Real-time Data - FIXED for your table structure
// class DashboardContent extends StatefulWidget {
//   final Map<String, dynamic> userData;
//   const DashboardContent({super.key, required this.userData});
...
//   // District list (simplified - no taluka grouping since taluka doesn't exist)
...
//   // Sales data storage - store orders directly
//   List<Map<String, dynamic>> _districtOrders = [];
//   double _totalDistrictSales = 0.0;
...
//   // Employee list for the district
//   List<Map<String, dynamic>> _districtEmployees = [];
...
//       print('✅ Found ${_districtEmployees.length} employees in district $_selectedDistrict');
...
//           .subscribe((status, [error]) {
//             if (status == 'SUBSCRIBED') {
//               print('✅ Orders channel subscribed for district $_selectedDistrict');
//             } else if (status == 'CHANNEL_ERROR') {
//               print('❌ Orders channel error: $error');
//               _setupPollingFallback();
//             }
...
//       print('📊 Loading REAL sales data for district: $_selectedDistrict');
...
//       print('📅 Date range: ${firstDayOfMonth.toIso8601String()} to ${lastDayOfMonth.toIso8601String()}');
...
//       print('📦 Found ${allOrders.length} completed orders for district $_selectedDistrict');
      
//       // Store orders
//       _districtOrders = List<Map<String, dynamic>>.from(allOrders);
...
//         if (_totalOrders <= 10) {
//           print('   Order ${_totalOrders}: ${order['customer_name']} - ₹$price - ${weightInTons.toStringAsFixed(2)} T - ${order['bags']} bags');
//         }
//       }
      
//       print('💰 TOTAL REAL Revenue: ₹${_totalRevenue.toStringAsFixed(2)}');
//       print('📊 TOTAL REAL Orders: $_totalOrders');
//       print('📦 TOTAL REAL Bags: $_totalBags');
//       print('⚖️ TOTAL REAL Weight: ${_totalDistrictSales.toStringAsFixed(2)} T');
      
//       // Update target with real revenue and orders
//       if (_currentTargetData != null) {
//         setState(() {
//           _currentTargetData!['achieved_revenue'] = _totalRevenue;
//           _currentTargetData!['achieved_orders'] = _totalOrders.toDouble();
...
//     // Calculate from bags * weight_per_bag if total_weight not available
//     final bags = (order['bags'] as num?)?.toInt() ?? 0;
    
//     // Weight per bag is not in the orders table, so we need to estimate
//     // Based on typical bag weight for cattle feed
//     // You can adjust this based on your actual data
//     final typicalBagWeightKg = 50.0; // 50 kg per bag standard
    
//     weightInKg = bags * typicalBagWeightKg;
...
//       // Add revenue and order targets if not present
//       if (targetData != null) {
//         if (!targetData.containsKey('revenue_target')) {
//           targetData['revenue_target'] = 0.0;
//         }
//         if (!targetData.containsKey('order_target')) {
//           targetData['order_target'] = 0.0;
//         }
//         if (!targetData.containsKey('achieved_revenue')) {
//           targetData['achieved_revenue'] = _totalRevenue;
//         }
//         if (!targetData.containsKey('achieved_orders')) {
//           targetData['achieved_orders'] = _totalOrders.toDouble();
//         }
//       }
...
//       print('🎯 Target Loaded: Revenue Target: ${targetData?['revenue_target'] ?? 0}, Order Target: ${targetData?['order_target'] ?? 0}');
...
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(
//             content: Text('✅ Dashboard updated: $_totalOrders orders, ₹${_totalRevenue.toStringAsFixed(0)} revenue'),
//             backgroundColor: Colors.green,
...
//             itemCount: _districtOrders.take(5).length,
//             separatorBuilder: (_, __) => const Divider(),
//             itemBuilder: (context, index) {
//               final order = _districtOrders[index];
//               return ListTile(
...
//                                 Container(width: 6, height: 6, decoration: const BoxDecoration(color: Colors.green, shape: BoxShape.circle)),
//                                 const SizedBox(width: 4),
//                                 Text('Auto-refresh every 30s', style: TextStyle(fontSize: 9, color: Colors.green[700], fontWeight: FontWeight.w500)),
//                               ],
//                             ),
//                           ),
//                         ],
...
//                                 Container(width: 8, height: 8, decoration: const BoxDecoration(color: Colors.green, shape: BoxShape.circle)),
//                                 const SizedBox(width: 6),
//                                 Text("Live updates from completed orders", style: GoogleFonts.poppins(color: Colors.white.withOpacity(0.8), fontSize: 10)),
//                               ],
//                             ),
//                           ],
//                         ),
...
//                         child: Column(
//                           children: [
//                             Text("Total Orders", style: TextStyle(fontSize: 12, color: Colors.grey[600])),
//                             const SizedBox(height: 4),
//                             Text(_totalOrders.toString(), 
//                               style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: GlobalColors.primaryBlue)),
//                           ],
//                         ),
//                       ),
//                       Container(width: 1, height: 40, color: Colors.grey.shade300),
...
//                         child: Column(
//                           children: [
//                             Text("Total Bags", style: TextStyle(fontSize: 12, color: Colors.grey[600])),
//                             const SizedBox(height: 4),
//                             Text(NumberFormat.compact().format(_totalBags), 
