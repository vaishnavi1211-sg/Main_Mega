...
class _MarketingManagerDashboardState extends State<MarketingManagerDashboard> {
  ...
  final List<Widget> _pages = [
    DashboardContent(userData: {}), // Pass userData
      ...
      title: Column(
        ...
        children: [
          ...
          if (widget.userData.isNotEmpty) 
            Text(
              widget.userData['full_name'] ?? 'Manager',
              style: GoogleFonts.poppins(
...
class _DashboardContentState extends State<DashboardContent> {
  ...
  Future<void> _loadCurrentTarget() async {
    ...
    try {
      final managerId = widget.userData['id'] ?? widget.userData['emp_id'];
      if (managerId == null) {
        setState(() => _isLoadingTarget = false);
        return;
      }
  ...
  Widget _buildAssignedTargetCard() {
    ...
    final revenueProgress = target.achievedRevenue / target.revenueTarget;
    final orderProgress = target.achievedOrders / target.orderTarget;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            "Updated: ${DateFormat('dd MMM yyyy, hh:mm a').format(DateTime.now())}",
            style: TextStyle(
              fontSize: 10,
              color: Colors.grey[500],
