...
class _MarketingProfilePageState extends State<MarketingProfilePage> {
    ...
    try {
      ...
      if (data == null) {
        ...
        if (emailData != null) {
          ...
          _createDefaultProfile(user.email ?? 'manager@mega.com');
        }
      } else {
  ...
  void _processUserData(Map<String, dynamic> data) {
    setState(() {
      managerData = {
        'empId': data['emp_id']?.toString() ?? 'N/A',
        'empName': data['full_name']?.toString() ?? 'Marketing Manager',
        'position': data['position']?.toString() ?? 'Marketing Manager',
        'branch': data['branch']?.toString() ?? 'Head Office',
        'district': data['district']?.toString() ?? 'N/A',
        'joiningDate': data['joining_date'] != null 
            ? DateTime.parse(data['joining_date'].toString()) 
            : DateTime.now(),
        'status': data['status']?.toString() ?? 'Active',
        'phone': data['phone']?.toString() ?? 'Not Provided',
        'email': data['email']?.toString() ?? 'N/A',
        'role': data['role']?.toString() ?? 'Marketing Manager',
  ...
  Widget _buildProfileHeader(Map<String, dynamic> manager) {
    final empName = manager['empName']?.toString() ?? 'Marketing Manager';
    final empNameInitials = empName.isNotEmpty && empName.length >= 2 
  ...
  Widget _buildManagerIdCard(Map<String, dynamic> manager) {
    final joiningDate = manager['joiningDate'];
    final formattedDate = joiningDate is DateTime 
  ...
  // ================= DETAILS SECTION =================
  Widget _buildDetailsSection(Map<String, dynamic> manager) {
    final joiningDateObj = manager['joiningDate'];
    final formattedJoiningDate = joiningDateObj is DateTime 
        ? DateFormat('dd MMMM yyyy').format(joiningDateObj)
        : DateFormat('dd MMMM yyyy').format(DateTime.now());
    
    // Calculate salary value safely
    final salaryValue = manager['salary'];
    final bool showSalary = salaryValue is num && salaryValue.toInt() > 0;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (manager['role'] != null && manager['role'].toString().isNotEmpty) ...[
                const Divider(),
                _infoItem(Icons.work, "Role", manager['role']?.toString() ?? 'N/A'),
              ],
              if (showSalary) ...[
                const Divider(),
                _infoItem(Icons.currency_rupee, "Salary", "₹$salaryValue"),
              ],
            ],
          ),
        ),
